...
func (db *appdbimpl) CheckLikeExists(user_id uint64, photo_id uint64) (bool, error) {
	// Eseguire una query per verificare se il like esiste
	query := `SELECT COUNT(*) FROM Like WHERE user_id = ? and photo_id = ?`
	var exist int
	err := db.c.QueryRow(query, user_id, photo_id).Scan(&exist)
	if err != nil {
...
func (db *appdbimpl) DeleteLike(user_id uint64, photo_id uint64) error {
...
func (db *appdbimpl) LikeCounterPhoto(photo_id uint64) (uint64, error) {
